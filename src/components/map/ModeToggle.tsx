...
export default function ModeToggle() {
  ...
  return (
    <div className="absolute top-3 left-1/2 -translate-x-1/2 z-[1000] w-[calc(100%-6rem)] md:w-auto">
      <div className="flex flex-col bg-[#1a1a1a]/90 backdrop-blur-xl rounded-2xl border border-white/10 shadow-2xl">
        {/* Toggle tabs */}
        <div className="flex">
          <button
            onClick={handleSwitchToNearby}
            className={`flex-1 md:px-6 px-4 py-2 text-xs font-bold transition-all cursor-pointer ${
              tripMode === "nearby"
            ...
            >
              <div className="px-2 pb-2 pt-1 relative">
                ...
                {destResults.length > 0 && (
                  <div className="absolute left-2 right-2 top-full mt-1 rounded-xl border border-white/10 bg-[#242424] shadow-2xl overflow-hidden z-[2000]">
                    {destResults.map((r, i) => (
                      <button
                        key={i}
